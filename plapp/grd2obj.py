...
def sph2cart(r, lat, lon):
    ...
    """
    x = r * np.cosd(lon) * np.cosd(lat)
    y = r * np.sind(lon) * np.cosd(lat)
    z = r * np.sind(lat)
...
def unproject_stereographic(x, y, lon0, lat0, R):
    ...
    lat = np.rad2deg(np.arcsin(np.cos(c) * np.sind(lat0) + (np.cosd(lat0) * y * np.sin(c)) / rho))
    lon = np.mod(
        lon0 + np.rad2deg(np.arctan2(x * np.sin(c), np.cosd(lat0) * rho * np.cos(c) - np.sind(lat0) * y * np.sin(c))), 360)
