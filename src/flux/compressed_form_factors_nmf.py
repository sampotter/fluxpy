...
class FormFactorNmfBlock(FormFactorLeafBlock,
    ...
    def __init__(self, root, w, h):
        ...
        self._w = w if nbytes(w) < nbytes(w_csr) else w_csr
    ...
    @property
    def nbytes(self):
        return nbytes(self._w) + nbytes(self._h)

    @property
    def compressed(self):
        return isinstance(self._w, scipy.sparse.spmatrix) \
            or isinstance(self._h, scipy.sparse.spmatrix)

    def _get_sparsity(self, tol=None):
        w_nnz = flux.linalg.nnz(self._w, tol)
        h_nnz = flux.linalg.nnz(self._h, tol)
        size = self._w.size + self._h.size
        return 0 if size == 0 else (w_nnz + h_nnz)/size
...
class FormFactorSparseNmfBlock(FormFactorLeafBlock,
    ...
    @property
    def nbytes(self):
        return nbytes(self._w) + nbytes(self._h) + nbytes(self._sr)
    ...
    def _get_sparsity(self, tol=None):
        ...
        h_nnz = flux.linalg.nnz(self._h, tol)
        sr_nnz = flux.linalg.nnz(self._sr, tol)
        size = self._w.size + self._h.size + self._sr.size
...
class CompressedFormFactorMatrix(scipy.sparse.linalg.LinearOperator):
    ...
    def __init__(self, shape_model, tol=1e-5,
        ...
        if compression_type == "wsnmf":
            ...
            E_t = np.array(E_t)
            weights = E_t.mean(axis=0) / E_t.mean(axis=0).max()
            self._FF_weights = np.outer(weights, weights)
