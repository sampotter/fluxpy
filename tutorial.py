...
if __name__ == '__main__':
    ...
    x0, x1, y0, y1 = -35, 25, 40, 100 # Bounding box of crater (not
                                      ...
                                      # the moment)
    h = 0.49 # Desired edge lengths of triangles generated by DistMesh
    ...
    # Since Embree only runs in single precision, there's no reason to
    # keep any variables in double precision.
    ...
    if os.path.isfile('FF.bin'):
        ...
        if FF.shape[0] != num_faces or FF.shape[1] != num_faces:
            print('FF.bin has wrong size: found %d faces but have %d faces' % (
                (FF.shape[0], num_faces)))
        FF = None
    if FF is None:
        # Build the compressed form factor matrix. All of the code related
        # to this can be found in the "form_factors.py" file in this
        # directory.
        FF = FormFactorMatrix.assemble_using_quadtree(scene, V, F, tol=1e-4)
