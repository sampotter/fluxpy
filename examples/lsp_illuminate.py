...
def illuminate_form_factor(FF_path = 'lsp_compressed_form_factors.bin', compressed=True, plot_fluxes=False,
    ...
    if use_svd:
        ...
        if engine == 'cgal':
            shape_model = CgalTrimeshShapeModel(V.copy(order='C'), F.copy(order='C'), N.copy(order='C')) #TrimeshShapeModel(V, F, N, P)
        elif engine == 'embree':
            shape_model = EmbreeTrimeshShapeModel(V.copy(order='C'), F.copy(order='C'), N.copy(order='C'))  # TrimeshShapeModel(V, F, N, P)
        else:
